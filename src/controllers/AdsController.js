...
const addImage = async (buffer) => {
    ...
    tmpImg.cover(500, 250).quality(80).write(`./public/media/${newName}`);
    return newName;
}
...
module.exports = {
    ...
    getList: async (req, res) => {
        ...
        const adsTodal = await Ad.find(filters).exec();
        total = adsTodal.length;

        const adsData = await Ad.find(filters)
        .sort({dataCreated: (sort == 'desc' ? -1 : 1)}).skip(parseInt(offset)).limit(parseInt(limit)).exec();
    ...
    editAction: async (req, res) => {
        ...
        if(cat) {
            ...
            ad.category = category._id.toString();
        }

        let result = await ad.save();

        res.json({ info: 'Atualizado com sucesso', result});
        return;
    }
};